...
/**
 * Low-res tier of every map, generated by scripts/make-low-tier.sh:
 * same filename under /textures/low/, 1/8 width, a few KB each.
 */
function lowTierUrl(url: string): string {
  return url.replace('/textures/', '/textures/low/')
}

const configureSurface = (t: THREE.Texture): void => {
  t.colorSpace = THREE.SRGBColorSpace
  t.anisotropy = 4
  t.wrapS = THREE.RepeatWrapping // equirectangular maps wrap in longitude
}

const configureBackground = (t: THREE.Texture): void => {
  t.colorSpace = THREE.SRGBColorSpace
  t.mapping = THREE.EquirectangularReflectionMapping
}

/**
 * Progressive two-tier loading: the low-res tier paints almost
 * immediately, the full map replaces it when fetched. Returns null until
 * either tier has loaded (or when no URL is given) — callers keep a
 * procedural fallback for that window, which also means materials never
 * start without a map (the SOL-25 late-texture gotcha).
 */
function useProgressiveTexture(
  url: string | undefined,
  configure: (t: THREE.Texture) => void,
): THREE.Texture | null {
  const fromCache = url ? (imageCache.get(url) ?? imageCache.get(lowTierUrl(url)) ?? null) : null
  ...
  useEffect(() => {
    ...
    const low = lowTierUrl(url)
    if (low !== url && !imageCache.has(low)) {
      loader.load(
        low,
        (t) => {
          configure(t)
          imageCache.set(low, t)
          // the full map may have won the race — never downgrade
          if (alive && !fullArrived) setTexture(t)
        },
        undefined,
        () => {
          // low tier missing: the full map is still on its way
        },
      )
    }
    loader.load(
      ...
      undefined,
      () => {
        // load error: keep the low tier or the caller's procedural fallback
      },
    )
    return () => {
      alive = false
 ...
 * CC BY 4.0, served from /public/textures), low-res tier first.
 * Returns null until a tier has loaded or when no URL is given —
 * callers fall back to procedural textures, which also covers custom
 * planets and offline use.
 */
export function useImageTexture(url: string | undefined): THREE.Texture | null {
  return useProgressiveTexture(url, configureSurface)
}
...
export function useBackgroundTexture(url: string): THREE.Texture | null {
  return useProgressiveTexture(url, configureBackground)
}
...
// keyed by texture instance, not url: with progressive loading the same
// url resolves to a low-res texture first and the full map later, and
// each tier needs its own bake
const shapedCloudCache = new WeakMap<THREE.Texture, THREE.Texture>()
...
export function useCloudAlphaTexture(url: string): THREE.Texture | null {
  ...
  return useMemo(() => {
    if (!base) return null
    const cached = shapedCloudCache.get(base)
    ...
    texture.wrapS = THREE.RepeatWrapping
    shapedCloudCache.set(base, texture)
    return texture
  }, [base])
}
